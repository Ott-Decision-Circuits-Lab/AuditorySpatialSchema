function sma = AuditorySpatialSchema_StateMatrix(iTrial)
...
CenterValveTime  = GetValveTimes(TrialData.RewardMagnitude(2,iTrial), CenterPort);
RightValveTime  = GetValveTimes(TrialData.RewardMagnitude(3,iTrial), RightPort);


%correct/incorrect choice to L/R mapping
%TO may 2025
LInStateChange = 'IncorrectChoice';
CInStateChange = 'IncorrectChoice';
RInStateChange = 'IncorrectChoice';
switch TrialData.CorrectLocation(iTrial)
    case 1 %left is correct
        LInStateChange = 'WaterL';
    case 3 %center is correct
        CInStateChange = 'WaterC';
    case 5 %right is correct
...
sma = AddState(sma, 'Name', 'PreITI',...
    ...
    'StateChangeConditions', {'Tup', 'WaitHIn'},...
    'OutputActions', PreITIAction);

sma = AddState(sma, 'Name', 'WaitHIn',...
    'Timer', TaskParameters.GUI.WaitCInMax,...
    'StateChangeConditions', {HomePortIn, 'StartHIn',...
                              'Tup', 'NoTrialStart'},...
    'OutputActions', {HomeLight, 255});
...
sma = AddState(sma, 'Name', 'StartHIn',... % dummy state for trigger GlobalTimer1
    'Timer', 0,...
...
sma = AddState(sma, 'Name', 'StimulusDelay',...
    ...
    'StateChangeConditions', {'Tup', 'Sampling',...
                              HomePortOut, 'BrokeFixation',...
                              'GlobalTimer1_End', 'StillSampling'},...
    'OutputActions', {});

TupStateChange = 'ITI';
if TaskParameters.GUI.RenewBrokeFixation
    TupStateChange = 'WaitHIn';
end
BrokeFixationAction = {};
...
%%
HomeInStateChange = 'WaitSIn';
HomeLightValue = 0;
if TaskParameters.GUI.StartNewTrialEnable
    HomeInStateChange = 'StartNewTrial';
    HomeLightValue = 255;
end
...
sma = AddState(sma, 'Name', 'StillSampling',... % dummy state for trigger GlobalTimer2
    ...
    'OutputActions', {'GlobalTimerTrig', 2,...
                      ...
                      HomeLight, HomeLightValue});

sma = AddState(sma, 'Name', 'WaitSIn',...
    'Timer', TaskParameters.GUI.ChoiceDeadline,...
    'StateChangeConditions', {'Tup', 'NoDecision',...
                              'GlobalTimer2_End', 'NoDecision',...
                              HomePortIn, HomeInStateChange,...
                              LeftPortIn, 'StartLIn',...
                              CenterPortIn, 'StartCIn',...
...
sma = AddState(sma, 'Name', 'StartNewTrial',...
    ...
    'StateChangeConditions', {'Tup', 'StartNewTrialTimeOut',...
                              HomePortOut, 'WaitSIn'},... %'GlobalTimer2_End', 'NoDecision'!?
    'OutputActions', {HomeLight, 255});
...
FeedbackDelayCenter = TrialData.FeedbackDelay(iTrial);
FeedbackDelayRight = TrialData.FeedbackDelay(iTrial);
if TaskParameters.GUI.CatchTrial

    error('Catch Trial not implemented for spatial schema task!')
...
sma = AddState(sma, 'Name', 'LInGrace',...
    ...
    'StateChangeConditions', {LeftPortIn, 'LIn',...
                              ...
                              HomePortIn, 'SkippedFeedback',...
                              RightPortIn, 'SkippedFeedback'},...
    'OutputActions', {LeftLight, LeftLightValue});

sma = SetGlobalTimer(sma, 4, FeedbackDelayCenter); % used to track side poke grace period

sma = AddState(sma, 'Name', 'StartCIn',... % dummy state for trigger GlobalTimer3
    'Timer', 0,...
    'StateChangeConditions', {'Tup', 'CIn',...
                              'GlobalTimer4_End', CInStateChange},...
    'OutputActions',{'GlobalTimerTrig', 4, CenterLight, CenterLightValue});

sma = AddState(sma, 'Name', 'CIn',...
    'Timer', FeedbackDelayCenter,...
    'StateChangeConditions', {'Tup', CInStateChange,...
                              'GlobalTimer4_End', CInStateChange,...
                              CenterPortOut,'CInGrace'},...
    'OutputActions', {CenterLight, CenterLightValue});

sma = AddState(sma, 'Name', 'WaterC',...
    'Timer', CenterValveTime,...
    'StateChangeConditions', {'Tup', 'Drinking'},...
    'OutputActions', {'ValveState', CenterValve});

sma = AddState(sma, 'Name', 'CInGrace',...
    'Timer', TaskParameters.GUI.FeedbackDelayGrace,...
    'StateChangeConditions', {LeftPortIn, 'CIn',...
                              'Tup', 'SkippedFeedback',...
...
sma = SetGlobalTimer(sma, 5, FeedbackDelayRight); % used to track side poke grace period


sma = AddState(sma, 'Name', 'StartRIn',... % dummy state for trigger GlobalTimer3
    'Timer', 0,...
    'StateChangeConditions', {'Tup', 'RIn',...
                              'GlobalTimer5_End', RInStateChange},...
    'OutputActions',{'GlobalTimerTrig', 5, RightLight, RightLightValue});
...
sma = AddState(sma, 'Name', 'RInGrace',...
    ...
    'StateChangeConditions', {RightPortIn, 'RIn',...
                              'Tup', 'SkippedFeedback',...
                              'GlobalTimer5_End', 'SkippedFeedback',...
                              CenterPortIn, 'SkippedFeedback',...
                              HomePortIn, 'SkippedFeedback',...
                              LeftPortIn, 'SkippedFeedback'},...
    'OutputActions', {RightLight, RightLightValue});
